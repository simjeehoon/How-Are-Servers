...
import customInterpreter
import customReportWriter
...
class InitialSetter:
    ...
    def __init__(self, command_directory_path: str):
...
class InterpreterSet:
    def get_interpreter(self, to_do_keyword):
        try:
            result = eval(f"customInterpreter.{to_do_keyword}()")
        except:
...
class Worker:
    def __init__(self, server_count: int, config: Config, printer: Printer,
        ...
        if self.interpreter_set is not None:
            self.report_writer = customReportWriter.ReportWriter(server_count, report_file_path, config.default_encoding)
...
class Main:
    ...
    def init(self):
        log.info("프로그램 시작")
        setter = InitialSetter(command_directory_path)
        setter.start()
    ...
    def run(self, with_save_txt: bool, with_interpreter: bool, single_thread_mode:bool):
        try:
            ...
            if len(serverInfoList) > 0:
                ...
                if with_interpreter:
                    interpreter_set = InterpreterSet()
                else:
